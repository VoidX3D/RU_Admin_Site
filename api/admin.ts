...
async function handleAction(action: string, params: any) {
  switch (action) {
    ...
    case 'missions:save': {
      ...
      await supabaseAdmin.from('missions').upsert({ id, ...missionFields })
      if (stats !== undefined) {
        await supabaseAdmin.from('mission_stats').delete().eq('mission_id', id)
        if (Array.isArray(stats) && stats.length > 0) {
          await supabaseAdmin.from('mission_stats').insert((stats as any[]).map((s, i) => ({ mission_id: id, label: s.label, value: s.value, sort_order: i })))
        }
      }
      if (partners !== undefined) {
        await supabaseAdmin.from('mission_partners').delete().eq('mission_id', id)
        if (Array.isArray(partners) && partners.length > 0) {
          await supabaseAdmin.from('mission_partners').insert((partners as string[]).map((name, i) => ({ mission_id: id, name, sort_order: i })))
        }
      }
      if (images !== undefined) {
        await supabaseAdmin.from('mission_images').delete().eq('mission_id', id)
        if (Array.isArray(images) && images.length > 0) {
          await supabaseAdmin.from('mission_images').insert((images as string[]).map((url, i) => ({
      ...
      if (goals !== undefined) {
        ...
        if (Array.isArray(goals) && goals.length > 0) {
          await supabaseAdmin.from('mission_goals').insert((goals as string[]).map((g, i) => ({ mission_id: id, goal: g, sort_order: i })))
        }
      }
      if (timeline !== undefined) {
        await supabaseAdmin.from('mission_timeline').delete().eq('mission_id', id)
        if (Array.isArray(timeline) && timeline.length > 0) {
          await supabaseAdmin.from('mission_timeline').insert((timeline as any[]).map((t, i) => ({
      ...
      if (participants !== undefined) {
        ...
        if (Array.isArray(participants) && participants.length > 0) {
          await supabaseAdmin.from('mission_participants').insert((participants as any[]).map((p, i) => ({
      ...
      if (budget !== undefined) {
        ...
        if (Array.isArray(budget) && budget.length > 0) {
          await supabaseAdmin.from('mission_budget').insert((budget as any[]).map((b, i) => ({
    ...
    case 'announcements:detail': {
      ...
      return {
        data: {
          ...announcement,
          image: announcement.image ? storageUrl(announcement.image.startsWith('announcements/') ? announcement.image : `announcements/${announcement.image}`) : null,
    ...
    case 'announcements:save': {
      ...
      await supabaseAdmin.from('announcements').upsert({ id, ...dataFields })
      if (tags !== undefined) {
        await supabaseAdmin.from('announcement_tags').delete().eq('announcement_id', id)
        if (Array.isArray(tags) && tags.length > 0) {
          await supabaseAdmin.from('announcement_tags').insert((tags as string[]).map((tag, i) => ({ announcement_id: id, tag, sort_order: i })))
        }
      }
      if (gallery !== undefined) {
        await supabaseAdmin.from('announcement_gallery').delete().eq('announcement_id', id)
        if (Array.isArray(gallery) && gallery.length > 0) {
          await supabaseAdmin.from('announcement_gallery').insert((gallery as string[]).map((url, i) => {
