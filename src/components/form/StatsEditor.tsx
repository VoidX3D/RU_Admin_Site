      ...
      <div className="space-y-2">
        {stats.map((s, i) => (
          <div key={i} className="flex items-center gap-2">
            <input value={s.label} onChange={e => update(i, 'label', e.target.value)} placeholder={keyPlaceholder}
              className="w-full rounded-lg border dark:border-zinc-800 dark:bg-zinc-900/50 px-3 py-2 text-xs text-zinc-900 dark:text-white outline-none placeholder:text-zinc-400 focus:border-emerald-500/50" />
