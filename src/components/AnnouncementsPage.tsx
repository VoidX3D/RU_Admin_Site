...
export function AnnouncementsPage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      saveDraft(`announcement_${fId}`, {
        id: fId,
        type: 'announcement',
        data: { title: fTitle, tag: fTag, status: fStatus, date: fDate, day: fDay, issued_by: fIssued, summary: fSummary, description: fDesc, importance: fImport, instructions: fInstr, active: fActive, deadline: fDeadline, tags: fTags, image: fImg?.dataUrl },
        savedAt: Date.now(),
        label: fTitle || 'Untitled Announcement',
      })
      setDraftSavedAt(Date.now())
    }, 30000)
    return () => clearInterval(interval)
  }, [mode, fId, fTitle, fTag, fStatus, fDate, fDay, fIssued, fSummary, fDesc, fImport, fInstr, fActive, fDeadline, fTags, fImg])
  ...
  function startNew() {
    ...
    if (draft) {
      ...
      if (d.status) setFStatus(d.status)
      if (d.date) setFDate(d.date)
      if (d.day) setFDay(d.day)
      if (d.issued_by) setFIssued(d.issued_by)
      if (d.summary) setFSummary(d.summary)
      if (d.description) setFDesc(d.description)
      if (d.importance) setFImport(d.importance)
      if (d.instructions) setFInstr(d.instructions)
      if (d.active !== undefined) setFActive(d.active)
      if (d.deadline) setFDeadline(d.deadline)
      if (d.tags) setFTags(d.tags)
      if (d.image) setFImg({ dataUrl: d.image, name: 'draft', remote: d.image.startsWith('http') })
  ...
  function validate(): boolean {
    ...
    if (fTitle.trim().length > 120) errs.title = 'Title must be under 120 characters'
    if (fSummary.trim().length > 300) errs.summary = 'Summary must be under 300 characters'
    setErrors(errs)
  ...
  function clearDraft() {
    if (fId) removeDraft(`announcement_${fId}`)
    setDraftSavedAt(null)
  ...
  if (mode === 'form') {
    return (
      <PageErrorBoundary name="Announcement Form" onReset={() => setMode('list')}>
        <motion.div
            ...
            <div className="rounded-xl border dark:border-zinc-800/50 dark:bg-zinc-900/30">
              ...
              <div className="p-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <button
                    className="rounded-lg bg-emerald-500 px-3 py-2 text-xs font-semibold text-white hover:bg-emerald-400 min-h-[44px] sm:min-h-0"
                    onClick={() => {
            ...
            <div className="rounded-xl border dark:border-zinc-800/50 dark:bg-zinc-900/30">
              ...
              <div className="p-4">
                <ImageUpload images={fImg ? [fImg] : []} onChange={imgs => setFImg(imgs[0] || null)} single maxCount={1}
                  pathPrefix="announcements" bucket="public" />
              </div>
            </div>
          </div>
        </motion.div>
  ...
  return (
    <PageErrorBoundary name="Announcements List">
      <motion.div
        initial={{ opacity: 0, y: 8 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.25, ease: [0.16, 1, 0.3, 1] }}
                    ...
                    <tr key={a.id} className="transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-800/20"
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <button className="flex items-center gap-1 rounded-lg px-3 py-1.5 text-[10px] font-medium dark:text-white hover:bg-zinc-200 dark:bg-zinc-800 hover:text-zinc-700 min-h-[44px] sm:min-h-0" onClick={() => startEdit(a.id)}>
                            <EditIcon size={12} /> Edit
                          </button>
                          <button title="Delete announcement" className="rounded-lg p-2 dark:text-zinc-700 hover:bg-red-100 dark:bg-red-500/10 hover:text-red-600 min-h-[44px] min-w-[44px] sm:min-h-0 sm:min-w-0 sm:p-1" onClick={() => handleDelete(a.id)}>
                            <TrashIcon size={12} />
                          </button>
                        </div>
                      </td>
